...
for ano in range(Config.ANO_FINAL, Config.ANO_INICIAL-1, -1):
    ...
    nome_arquivo = f"parcelamento-por-data-emissao-{str_data_inicio}-{str_data_fim}.xls".replace("/", ".")
    ...
    for _ in range(3):
        ...
        try:
            ...
            link_consulta_inscritos = get_element_by_xpath(
                driver,
                '//b[contains(text(), "- Consulta Parcelamentos") and contains(text(), "data de emissão")]//ancestor::a',
                max_wait=20,
            )
